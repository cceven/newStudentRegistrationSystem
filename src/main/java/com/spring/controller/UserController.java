...
public class UserController extends BaseController {
    ...
    protected String authLoginUser(boolean isAdmin, String username, String pwd, String cx) {
        ...
        if (cx.equals("新生")) {
            table = "xinsheng";
            Xinsheng user = xinshengService.login(username, pwd);
            if (user == null) {
                return showError("用户名或密码错误");
            }
            if (!user.getIssh().equals("是")) {
                return showError("系统查询到此学号新生暂未注册，无法登陆，请联系管理员！！！");
            }
            session.setAttribute("id", user.getId());
            ...
            session.setAttribute("shengyuandi", user.getShengyuandi());
            ...
            session.setAttribute("lianxifangshi", user.getLianxifangshi());
            session.setAttribute("xueshengjianjie", user.getXueshengjianjie());
            session.setAttribute("shifoufenpeisushe", user.getShifoufenpeisushe());
            session.setAttribute("suozaisushehao", user.getSuozaisushehao());
            session.setAttribute("chuangweihao", user.getChuangweihao());
            session.setAttribute("shifouyijiaoxuefei", user.getShifouyijiaoxuefei());
            session.setAttribute("sushejiaofei", user.getSushejiaofei());
        }
...
//            return showSuccess("登录成功", referer);
    ...
    public String editPassword() {
        ...
        if (cx.equals("新生")) {
            Xinsheng user = xinshengService.login(username, oldPassword);
            if (user == null) {
                return showError("原密码不正确");
            }
            xinshengService.updatePassword(user.getId(), newPwd);
        }
