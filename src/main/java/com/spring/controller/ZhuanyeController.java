...
public class ZhuanyeController extends BaseController {
    ...
    public String list() {
        ...
        List<Zhuanye> list = service.selectPageExample(example, page, 12); // 获取当前页的行数
        // 将列表写给界面使用
        assign("list", list);
        assign("orderby", order); // 把当前排序结果写进前台
        assign("sort", sort); // 把当前排序结果写进前台
        assign("where", where); // 把当前条件写给前台
        return "zhuanye_list"; // 使用视图文件：WebRoot\zhuanye_list.jsp
    }
    ...
    public String getWhere() {
        ...
        if (!Request.get("zhuanyemingcheng").equals("")) {
            where += " AND zhuanyemingcheng LIKE '%" + Request.get("zhuanyemingcheng") + "%' ";
        }
        return where;
    }
    ...
    public String insert() {
        ...
        // 设置前台提交上来的数据到实体类中
        post.setZhuanyemingcheng(Request.get("zhuanyemingcheng"));
    ...
    public String update() {
        ...
        // 将前台表单数据填充到实体类
        if (!Request.get("zhuanyemingcheng").equals("")) post.setZhuanyemingcheng(Request.get("zhuanyemingcheng"));
    ...
    public String delete() {
        ...
        Map map = Query.make("zhuanye").find(id);
