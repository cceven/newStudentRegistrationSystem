 ...
 * 调专业 */
@Controller
public class DiaozhuanyeController extends BaseController {

    @Autowired
    private DiaozhuanyeMapper dao;

    @Autowired
    private DiaozhuanyeService service;

    @Autowired
    private XinshengService serviceRead;

    /**
     *  后台列表页
     *
     */
    @RequestMapping("/diaozhuanye_list")
    public String list() {
        // 检测是否有登录，没登录则跳转到登录页面
        if (!checkLogin()) {
        ...
        Example example = new Example(Diaozhuanye.class); //  创建一个扩展搜索类
        ...
        List<Diaozhuanye> list = service.selectPageExample(example, page, 12); // 获取当前页的行数
        // 将列表写给界面使用
        assign("list", list);
        assign("orderby", order); // 把当前排序结果写进前台
        assign("sort", sort); // 把当前排序结果写进前台
        assign("where", where); // 把当前条件写给前台
        return "diaozhuanye_list"; // 使用视图文件：WebRoot\diaozhuanye_list.jsp
    }
    ...
    public String getWhere() {
        String where = " ";
        // 判断URL 参数xinshengid是否大于0
        if (Request.getInt("xinshengid") > 0) {
            // 大于0 则写入条件
            where += " AND xinshengid='" + Request.getInt("xinshengid") + "' ";
        }
     ...
     * 申请学生列表
     */
    @RequestMapping("/diaozhuanye_list_shenqingxuesheng")
    public String listshenqingxuesheng() {
        ...
        List<Diaozhuanye> list = service.selectPageExample(example, page, 12);

        request.setAttribute("list", list);
        assign("orderby", order);
        assign("sort", sort);
        assign("where", where);
        return "diaozhuanye_list_shenqingxuesheng";
    }

    @RequestMapping("/diaozhuanye_add")
    public String add() {
        int id = Request.getInt("id"); // 根据id 获取 新生模块中的数据
        Xinsheng readMap = serviceRead.find(id);
        // 将数据行写入给前台jsp页面
        request.setAttribute("readMap", readMap);
        return "diaozhuanye_add";
    }

    @RequestMapping("/diaozhuanye_updt")
    public String updt() {
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Diaozhuanye mmm = service.find(id);
        request.setAttribute("mmm", mmm);
        request.setAttribute("updtself", 0);
        return "diaozhuanye_updt";
    }
     ...
     * @return
     */
    @RequestMapping("/diaozhuanyeinsert")
    public String insert() {
        String tmp = "";
        Diaozhuanye post = new Diaozhuanye(); // 创建实体类
        // 设置前台提交上来的数据到实体类中
        post.setXinshengid(Request.getInt("xinshengid"));
     ...
     * @return
     */
    @RequestMapping("/diaozhuanyeupdate")
    public String update() {
        // 创建实体类
        Diaozhuanye post = new Diaozhuanye();
        // 将前台表单数据填充到实体类
        if (!Request.get("xinshengid").equals("")) post.setXinshengid(Request.getInt("xinshengid"));
        ...
        if (!Request.get("shenqingzhuangtai").equals("")) post.setShenqingzhuangtai(Request.get("shenqingzhuangtai"));
     ...
     *  后台详情
     */
    @RequestMapping("/diaozhuanye_detail")
    public String detail() {
        int id = Request.getInt("id");
        Diaozhuanye map = service.find(id); // 根据前台url 参数中的id获取行数据
        request.setAttribute("map", map); // 把数据写到前台
        return "diaozhuanye_detail"; // 详情页面：WebRoot\diaozhuanye_detail.jsp
    }

    /**
     *  删除
     */
    @RequestMapping("/diaozhuanye_delete")
    public String delete() {
        if (!checkLogin()) {
            return showError("尚未登录");
        }
        int id = Request.getInt("id"); // 根据id 删除某行数据
        Map map = Query.make("diaozhuanye").find(id);
